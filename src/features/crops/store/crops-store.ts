...
import {
  closeNewsTextModal,
  collectNewsIdsForCrops,
  consolidateNewsAfterCropMerge,
  findNewsByCropId,
  getLinkedNewsItems,
  linkCropToNews,
  openNewsTextModalFromCrop,
  registerCropsStore,
  splitNewsForUngroupedCrop,
  syncNewsCropLink,
  unhighlightLinkedNews,
} from '@/features/crop-news-linking/crop-news-coordinator'
import { finalizeNewsItemsOnPage, reopenPageInState } from './finalization'
import { sortCropIds } from './ordering'
import {
  loadPersistedCrops as loadPersisted,
  savePersistedCrops as savePersisted,
} from './persistence'
import { getRelatedCropIds } from './relations'
import type { CropsState } from './state-types'
import { combineGroupCropTexts } from './transformations'
...
export const useCropsStore = create<CropsState>((set, get) => ({
  ...
  addCropToNews: ({ editionId, pdfId, pageNumber, rect, newsItem }) => {
    ...
    linkCropToNews(newsItem.id, cropId)
    return cropId
  },
  ...
  deleteCrop: (cropId) => {
    ...
    const newsItemId =
      crop.newsItemId ?? findNewsByCropId(cropId)?.id ?? null
    ...
    if (openNewsId) {
      openNewsTextModalFromCrop(openNewsId)
    }
...
registerCropsStore({
  getState: useCropsStore.getState,
  setState: useCropsStore.setState,
})
